...
internal sealed class TicketRepository(AppDBContext context) : IBookingRepository
{
    ...
    public async Task<List<Booking>> GetOverlappingBookingsAsync(List<Guid> roomIds, DateTime checkIn,
        DateTime checkOut, CancellationToken ct = default)
    {
        // Konverter til UTC
    ...
    public async Task<Room?> GetAvailableRoomByTypeAsync(RoomType roomType, DateTime checkIn, DateTime checkOut,
        Guid? hotelId, List<Guid>? excludeRoomIds = null)
    {
        ...
        var availableRoom = await query
            .Where(r => !r.Bookings.Any(b =>
                ...
                b.CheckIn < checkOut &&
                b.CheckOut > checkIn))
            .OrderBy(r => r.Number) // s� vi altid f�r et deterministisk v�relse
            .FirstOrDefaultAsync();
