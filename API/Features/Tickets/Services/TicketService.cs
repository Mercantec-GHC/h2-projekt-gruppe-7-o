...
public class TicketService
{
    ...
    public TicketService(IBookingRepository repository, ILogger<BookingService> logger, MailService mailService)
    {
    ...
    public async Task<BookingResponseDto> CreateBookingAsync(BookingCreateDto dto, Guid userId, string userEmail,
        string userName)
    {
        ...
        foreach (var roomBookingDto in dto.RoomBookings)
        {
            ...
            if (room == null)
                throw new InvalidOperationException(
                    $"No rooms of type '{roomBookingDto.RoomType}' are available for the period.");
            usedRoomIds.Add(room.Id);

            if (room.Capacity < (roomBookingDto.Adults + roomBookingDto.Children))
            {
                throw new InvalidOperationException(
                    $"Room type '{room.Type}' does not have capacity for {roomBookingDto.Adults + roomBookingDto.Children} guests.");
            }
